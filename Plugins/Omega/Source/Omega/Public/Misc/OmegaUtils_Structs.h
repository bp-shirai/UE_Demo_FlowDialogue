...
struct FOmegaGlobalVarsContainer
{
    ...
    // UPROPERTY()
    // FJsonObjectWrapper main_vars;

    // UPROPERTY()
    // TMap<FGuid, FJsonObjectWrapper> vars_guid;

    // UPROPERTY()
    // TMap<FGameplayTag, FJsonObjectWrapper> vars_tag;
};
...
struct FOmegaSaveVars
{
    ...
    TMap<FGameplayTag, int32> params_int32;
...
struct FOmegaCustomObjectData
{
    ...
    UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Vars")
    TMap<FName, FVector> Params_num;

    UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Vars")
    TMap<FName, FString> Params_string;
};
