...
UAsyncAction_PushSoftWidget* UAsyncAction_PushSoftWidget::PushSoftWidget(const UObject* WorldContextObject, APlayerController* OwningPlayerController, TSoftClassPtr<UCommonActivatableWidget> InSoftWidgetClass, UPARAM(meta = (Categories = "Frontend.WidgetStack")) FGameplayTag InWidgetStackTag, bool bFocusOnNewlyPushedWidget)
{
...
{
    USimpleUIManager* UIManager = USimpleUIManager::Get(CachedOwningWorld.Get());

    UIManager->PushSoftWidgetToStackAsync(
        CachedWidgetStackTag, CachedSoftWidgetClass,
        [this](EAsyncPushWidgetState InPushState, UCommonActivatableWidget* PushedWidget)
        {
            switch (InPushState)
            {
