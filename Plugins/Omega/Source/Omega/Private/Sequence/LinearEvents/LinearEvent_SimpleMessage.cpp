...
#include "Sequence/LinearEvents/LinearEvent_SimpleMessage.h"
...
void ULinearEvent_SimpleMessage::GetGeneralDataText_Implementation(const FString& Label, const UObject* Context, FText& Name, FText& Description)
{
    Description = Message;
}
...
void ULinearEvent_SimpleMessage::Native_Begin(const FString& Flag)
{
    ...
    GetWorld()->GetGameInstance()->GetSubsystem<UOmegaGameManager>()->OnGlobalEvent.AddDynamic(this, &ThisClass::LocalGEvent);
...
void ULinearEvent_SimpleMessage::LocalGEvent(FName Event, UObject* Context)
{
    ...
    {
        GetWorld()->GetGameInstance()->GetSubsystem<UOmegaGameManager>()->OnGlobalEvent.RemoveDynamic(this, &ThisClass::LocalGEvent);
        Finish("");
    }
}
...
void UFlowNode_SimpleMessage::ExecuteInput(const FName& PinName)
{
    ...
    LocalMessage->EventEnded.AddDynamic(this, &ThisClass::LocalFinish);
    if (Instigator_Actor)
    {
        LocalMessage->Instigator_Actor = Instigator_Actor;
    }
    if (Instigator_Asset)
    {
        LocalMessage->Instigator_Asset = Instigator_Asset;
    }
